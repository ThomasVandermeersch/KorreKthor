...
const getExam = (includeCopiesAndUsers=false) => {
    return (req, res, next) => {
        query = includeCopiesAndUsers ? {where:{id:req.params.examid}, include:[{model:Copy, as:"copies", include:[{model:User, as:"user"}]}],order:[[{ model: Copy, as: 'copies' },'userMatricule', 'ASC']]} : {where:{id:req.params.examid}} ;
